    ...
    <div className="max-w-4xl">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <OrderItemsCard items={order.items.map((i) => ({ id: i.id, name: i.itemName, quantity: i.quantity }))} />
          <OrderPriceCard
...
function PaymentActions({ order }: { order: OrderDetailType }) {
  if (order.status !== OrderStatus.WAITING_FOR_PAYMENT || order.paymentStatus === PaymentStatus.PAID) {
    return null;
  }
  return (
    <>
      ...
      {order.payment?.status === PaymentStatus.PENDING && (
        <Link to={`/orders/${order.id}/pay`}>
          <Button variant="outline"><CreditCard className="h-4 w-4 mr-2" /> Complete Payment</Button>
        </Link>
      )}
