...
const CreateForm = ({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) => {
  ...
  const { mutate, isPending } = useCreateUser(() => { onSuccess(); onClose() })
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <FieldGroup className="mb-6">
        ...
        <Field>
          ...
          <Input id="outletId" placeholder="Outlet ID" {...register("outletId")} />
        </Field>
...
const EditForm = ({ user, onClose, onSuccess }: { user: User; onClose: () => void; onSuccess: () => void }) => {
  const { register, handleSubmit, setValue, control, formState: { errors } } =
  ...
  const { mutate, isPending } = useUpdateUser(() => { onSuccess(); onClose() })

  const onSubmit = (data: UpdateUserFormValues) =>
    mutate({ userId: user.id, payload: { ...data, outletId: data.outletId || undefined } })
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <FieldGroup className="mb-6">
        ...
        <Field data-invalid={!!errors.role}>
          ...
          <Select
            value={roleValue ?? ""}
            onValueChange={(val) => setValue("role", val as UpdateUserFormValues["role"])}
          >
            <SelectTrigger>
              <SelectValue placeholder="Select role" />
            </SelectTrigger>
...
type CreateProps = { mode: "create"; user?: never; open: boolean; onClose: () => void; onSuccess: () => void }
type EditProps   = { mode: "edit";   user: User;   open: boolean; onClose: () => void; onSuccess: () => void }

export const UserFormModal = ({ mode, user, open, onClose, onSuccess }: CreateProps | EditProps) => (
