  ...
  return (
    <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-muted-foreground">
        {DRIVER_COPY.paginationPagePrefix} {meta.page}{' '}
        {DRIVER_COPY.paginationOf} {meta.totalPages} • {meta.total}{' '}
        {DRIVER_COPY.paginationTotalSuffix}
      </p>
...
export function DriverDeliveryList({ page, onPageChange }: Props) {
  const { data, isLoading, isError } = useAvailableDeliveries({
    page,
    limit: 10,
  });
  ...
  if (!data || data.data.length === 0) {
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          {DRIVER_COPY.deliveryEmptyState}
        </CardContent>
      </Card>
    );
  }
        ...
        <Card key={item.id}>
          <CardContent className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-0.5">
              ...
              {(item.deliveryAddress.phone ?? item.customer.phone) && (
                <p className="text-xs text-muted-foreground">
                  {item.deliveryAddress.phone ?? item.customer.phone}
                </p>
              )}
              <p className="text-xs text-muted-foreground">
                {item.deliveryAddress.label} — {item.deliveryAddress.street},{' '}
            ...
            >
              {accept.isPending
                ? DRIVER_COPY.accepting
                : DRIVER_COPY.acceptButton}
            </Button>
          </CardContent>
        </Card>
      ))}
