...
import type { DriverActiveTask, DriverPickupTask } from '@/types/delivery';
...
function CustomerBlock({ task }: { task: TaskWithContact }) {
  return (
...
function AddressBlock({ task }: { task: TaskWithContact }) {
  return (
    <div>
      <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">
      ...
      <p className="text-sm">
        {task.address.label} — {task.address.street}
      </p>
      {task.address.province ? (
        <p className="text-xs text-muted-foreground">
          {task.address.city}, {task.address.province}
        </p>
      ) : (
        <p className="text-xs text-muted-foreground">{task.address.city}</p>
      )}
    </div>
  );
}
...
export function DriverActiveTaskCard() {
  const [activeTask, setActiveTask] = useState<DriverActiveTask | null>(
    readActiveTask,
  );
  ...
  return (
    <Card className="border-primary/40 bg-primary/5">
      ...
      <CardContent className="space-y-3">
        <CustomerBlock task={activeTask as TaskWithContact} />
        <AddressBlock task={activeTask as TaskWithContact} />
        <Button
          className="w-full"
          size="sm"
          disabled={isPending}
